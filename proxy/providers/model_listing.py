...
class ProviderModelInfo:
    ...
    # Max input context in tokens, when the provider advertises it (else unknown).
...
def extract_openrouter_tool_model_infos(
    ...
    for item in data:
        ...
        if supported_parameter_names.isdisjoint({"tools", "tool_choice"}):
            continue
        raw_context = _field(item, "context_length")
        context_window = (
            raw_context if isinstance(raw_context, int) and raw_context > 0 else None
        )
        model_infos.add(
            ProviderModelInfo(
                model_id=model_id,
