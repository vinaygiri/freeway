...
from .directives import apply_inline_directive, parse_directive_aliases
...
class ModelRouter:
    ...
    def __init__(self, settings: Settings):
        ...
        self._directive_aliases = parse_directive_aliases(
            getattr(settings, "model_directives", "")
        )
    ...
    def resolve_fallback_candidates(self) -> list[ResolvedModel]:
        """Resolve the configured ``MODEL_FALLBACKS`` chain into ordered candidates.

        Only well-formed ``provider/model`` refs for supported providers are kept;
        malformed entries are logged and skipped.
        """
        candidates: list[ResolvedModel] = []
        for raw in self._settings.model_fallbacks.split(","):
            ref = raw.strip()
            if not ref:
                continue
            provider_id, separator, provider_model = ref.partition("/")
            if (
                not separator
                or not provider_model
                or provider_id not in SUPPORTED_PROVIDER_IDS
            ):
                logger.warning("Ignoring invalid MODEL_FALLBACKS ref: {}", ref)
                continue
            candidates.append(self.resolve(ref))
        return candidates
    ...
    def resolve_messages_request(
        ...
        An inline ``@``-directive in the latest user turn overrides the routed
        model and is stripped from the (copied) prompt before forwarding.
        """
        routed = request.model_copy(deep=True)
        directive = apply_inline_directive(routed, self._directive_aliases)
        if directive.override_ref:
            logger.debug(
                "MODEL DIRECTIVE: '{}' overrides model; ignored={}",
                directive.override_ref,
                directive.ignored,
            )
        resolved = self.resolve(directive.override_ref or request.model)
